...
void showLcdInfo(const char *text, bool Streak=false, bool Smiley=false, bool Sad=false) {
  lcd.clear();
  char line1[17] = {0};
  char line2[17] = {0};
  char buffer1[17] = {0};
...
void loop() {
  ...
  if (!waitInput) {
    ...
    Serial.println("wait");
    String message = "Level:" + String(level) + " Streak:" + String(streak);
    ...
    if (pos == length) {
      ...
      if (levelPassed(level_pattern, user_input)) {
        showLcdInfo("Congratulations! Passed!", false, true);
        ...
        showLcdInfo("You Lost! Try Again!", false, false, true);
        streak = 0;
        error();
        playLevel(level_pattern);
      }
